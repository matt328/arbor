...
using HandlerFunc = std::function<void(std::shared_ptr<void>)>;
using HandlerFuncMap = std::unordered_map<std::string, std::vector<HandlerFunc>>;

struct ThreadLocalData {
  std::mutex mutex;
  std::unordered_map<std::type_index, HandlerFuncMap> handlers;
