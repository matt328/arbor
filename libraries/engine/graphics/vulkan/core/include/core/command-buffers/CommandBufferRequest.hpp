...
struct CommandBufferRequest {
  ...
  auto operator==(const CommandBufferRequest& other) const -> bool {
    return threadId == other.threadId && frameId == other.frameId && hashKey == other.hashKey &&
...
namespace std {
template <>
struct hash<arb::CommandBufferRequest> {
  auto operator()(const arb::CommandBufferRequest& req) const -> std::size_t {
    std::size_t h1 = std::hash<std::thread::id>{}(req.threadId);
    std::size_t h2 = std::hash<uint8_t>{}(req.frameId);
    std::size_t h3 = std::hash<size_t>{}(req.hashKey);
