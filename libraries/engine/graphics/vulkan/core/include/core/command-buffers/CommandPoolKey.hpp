...
struct CommandPoolKey {
  std::thread::id threadId;
  uint8_t frameId;
  QueueType queueType;

  auto operator==(const CommandPoolKey& other) const -> bool {
...
namespace std {
template <>
struct hash<arb::CommandPoolKey> {
  auto operator()(const arb::CommandPoolKey& key) const -> std::size_t {
