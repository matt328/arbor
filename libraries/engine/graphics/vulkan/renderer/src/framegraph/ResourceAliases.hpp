...
inline auto to_string(GlobalBufferAlias alias) -> std::string {
  switch (alias) {
    ...
    case GlobalBufferAlias::Animation:
      return "Animation";
    case arb::GlobalBufferAlias::Count:
      return "Count";
  }
}
...
template <>
struct std::formatter<arb::GlobalBufferAlias> {
  constexpr auto parse(std::format_parse_context& ctx) {
    return ctx.begin();
  }
  auto format(arb::GlobalBufferAlias alias, std::format_context& ctx) const {
...
template <>
struct std::formatter<arb::BufferAlias> {
  constexpr auto parse(std::format_parse_context& ctx) {
    return ctx.begin();
  }
  auto format(arb::BufferAlias alias, std::format_context& ctx) const {
    return std::format_to(ctx.out(), "{}", to_string(alias));
  }
};
...
namespace std {
template <>
struct hash<arb::BufferAliasVariant> {
  auto operator()(const arb::BufferAliasVariant& alias) const -> std::size_t {
    return std::visit(
        [](auto&& a) -> std::size_t {
