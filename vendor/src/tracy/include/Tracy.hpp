...
#define ZoneNamed(varname, active)                                                                 \
  static constexpr tracy::SourceLocationData TracyConcat(                                          \
      ...
      TracyLine){nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0};                       \
  tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, TracyLine),                      \
                            TRACY_CALLSTACK,                                                       \
                            active)
...
#define ZoneNamedC(varname, color, active)                                                         \
  static constexpr tracy::SourceLocationData TracyConcat(                                          \
      __tracy_source_location,                                                                     \
      TracyLine){nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, color};                   \
  tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, TracyLine),                      \
                            TRACY_CALLSTACK,                                                       \
                            active)
#define ZoneNamedNC(varname, name, color, active)                                                  \
  static constexpr tracy::SourceLocationData TracyConcat(                                          \
      __tracy_source_location,                                                                     \
      TracyLine){name, TracyFunction, TracyFile, (uint32_t)TracyLine, color};                      \
  tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, TracyLine),                      \
                            TRACY_CALLSTACK,                                                       \
                            active)

#define ZoneTransient(varname, active)                                                             \
  tracy::ScopedZone varname(TracyLine,                                                             \
                            TracyFile,                                                             \
                            strlen(TracyFile),                                                     \
                            TracyFunction,                                                         \
...
#define TracyLockable(type, varname)                                                               \
  tracy::Lockable<type> varname {                                                                  \
    []() -> const tracy::SourceLocationData* {                                                     \
      static constexpr tracy::SourceLocationData srcloc{nullptr,                                   \
                                                        #type " " #varname,                        \
                                                        TracyFile,                                 \
                                                        TracyLine,                                 \
                                                        0};                                        \
...
#define TracyMessageC(txt, size, color)                                                            \
  tracy::Profiler::MessageColor(txt, size, color, TRACY_CALLSTACK)
...
#define TracySecureAlloc(ptr, size)                                                                \
  tracy::Profiler::MemAllocCallstack(ptr, size, TRACY_CALLSTACK, true)
...
#define TracyFreeN(ptr, name)                                                                      \
  tracy::Profiler::MemFreeCallstackNamed(ptr, TRACY_CALLSTACK, false, name)
...
#define TracySecureFreeN(ptr, name)                                                                \
  tracy::Profiler::MemFreeCallstackNamed(ptr, TRACY_CALLSTACK, true, name)
#define TracySecureMemoryDiscard(name)                                                             \
  tracy::Profiler::MemDiscardCallstack(name, true, TRACY_CALLSTACK)
...
#define TracySecureAllocS(ptr, size, depth)                                                        \
  tracy::Profiler::MemAllocCallstack(ptr, size, depth, true)
...
#define TracyFreeNS(ptr, depth, name)                                                              \
  tracy::Profiler::MemFreeCallstackNamed(ptr, depth, false, name)
...
#define TracySecureFreeNS(ptr, depth, name)                                                        \
  tracy::Profiler::MemFreeCallstackNamed(ptr, depth, true, name)
#define TracySecureMemoryDiscardS(name, depth)                                                     \
  tracy::Profiler::MemDiscardCallstack(name, true, depth)
...
#define TracyMessageCS(txt, size, color, depth)                                                    \
  tracy::Profiler::MessageColor(txt, size, color, depth)
...
#define TracyParameterSetup(idx, name, isBool, val)                                                \
  tracy::Profiler::ParameterSetup(idx, name, isBool, val)
